...
test.describe('POST /auth/login', () => {
    ...
    test('não deve logar com senha incorreta', async ({auth}) => {
        ...
        const response = await auth.login({...user, password: 'wrong-password'})
    ...
    test('não deve logar quando o email não é informado', async ({auth}) => {
        ...
        const response = await auth.login({...user, email: undefined})
    ...
    test('não deve logar quando a senha não é informado', async ({auth}) => {
        const user = getUser()
        const respCreate = await auth.createUser(user)
        expect(respCreate.status()).toBe(201)

        const response = await auth.login({...user, password: undefined})
        expect(response.status()).toBe(400)
